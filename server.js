...
async function handler(req, res) {
  try {
    ...
    // چک مسیر
    if (pathname !== PUBLIC_RELAY_PATH && !pathname.startsWith(PUBLIC_RELAY_PATH + "/")) {
      res.statusCode = 404;
      return res.end("Not Found");
    }

    // ساخت مسیر برای سرور هدف
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 XHTTP Relay running | Target: ${TARGET_BASE} | Public Path: ${PUBLIC_RELAY_PATH}`);
});
